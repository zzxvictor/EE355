...
void run (HunterBook * hb, Monsterbook *mb, PalicoBook *pb, ChampionBook *cb)
{
	...
	std::cout<<"_-----------9"<<std::endl;
	cb->save(hb,pb);
	std::cout<<"_-----------10"<<std::endl;
	hb->save();
	std::cout<<"_-----------11"<<std::endl;
	mb->save();
	std::cout<<"_-----------12"<<std::endl;
	pb->save();
	std::cout<<"_-----------13"<<std::endl;
		...
		for (int i = 0; i<hb->getListSize(); i++)
		{
			for (int j = 0; j<pb->getListSize(); j++)
			{
				...
				if ((hX == pX)&&(hY == pY))
				{
					...
					Champion * cp = new Champion(hb->indexAccess(i), pb->indexAccess(i));
					cb->add(cp);
