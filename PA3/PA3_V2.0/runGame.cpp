...
void runFight(HunterBook * hb, Monsterbook *mb, PalicoBook *pb, ChampionBook *cb)
{
	...
	t[0] = std::thread([=]{hunterFightMonster(hb,mb);});
	//hunterFightMonster(hb, mb);
	t[1] = std::thread([=]{palicoFightMonster(pb,mb);});
	//palicoFightMonster(pb, mb);
	t[2] = std::thread([=]{ChampionFightMonster(cb,mb);});
...
void moveAround(HunterBook * hb, Monsterbook *mb, PalicoBook *pb, ChampionBook *cb)
{
	...
	int hunterSize = hb->getListSize();
	int monsterSize = mb->getListSize();
	int palicoSize = pb->getListSize();
	int championSize = cb->getListSize();

	std::thread *t = new std::thread[hunterSize + monsterSize + palicoSize + championSize];

	int j = 0;
	for (int i = 0; i< hunterSize; i++)
	{
		t[i] = std::thread([=]{moveObj(hb, j);});
		j++;
	}
	j = 0;
	for (int i = hunterSize; i< hunterSize + monsterSize; i++)
	{
		t[i] = std::thread([=]{moveObj(mb, j);});
		j++;
	}
	j = 0;
	for (int i = hunterSize + monsterSize; i< hunterSize + monsterSize + palicoSize; i++)
	{
		t[i] = std::thread([=]{moveObj(pb, j);});
		j++;
	}
	j = 0;
	for (int i = hunterSize + monsterSize + palicoSize; i< hunterSize + monsterSize + palicoSize + championSize; i++)
	{
		t[i] = std::thread([=]{moveObj(cb, j);});
		j++;
	}

	for (int i = 0; i < hunterSize + monsterSize + palicoSize + championSize; ++i) 
	{
        t[i].join();
    }
...
template <typename T> 
void moveObj(T* obj, int index)
{
	int addX = rand()%3 - 1;
	int addY = rand()%3 - 1;
	obj->indexAccess(index)->setXY(addX,addY);
}
				...
				if ((cX == mX)&&(cY == mY))
				{
					while (1)
					{
						...
						if (coin > 10)
						{
							...
							if (cb->indexAccess(i)->getPalico()->getHP()>0)
